...
if __name__ == '__main__':
    ...
    segmentation_network.load_weights('Weights/' + session_name + '/' + str(segmentation_network.model_name) + '/Epoch_' + str(epoch) + 'batch_' + str(batch_id) + '/')
    generator.load_weights('Weights/' + session_name + '/' + str(generator.model_name) + '/Epoch_' + str(epoch) + 'batch_' + str(batch_id) + '/')
    discriminator.load_weights('Weights/' + session_name + '/' + str(discriminator.model_name) + '/Epoch_' + str(epoch) + 'batch_' + str(batch_id) + '/')
    ...
    for batch_id, (batch_images_real, batch_labels) in enumerate(test_dataset):
        ...
        batch_images_fake, batch_regions_fake, z_k = generator(batch_images_real, batch_masks_logits, update_generator=True, training=True)
        print(z_k)
        z_k_hat = z_k

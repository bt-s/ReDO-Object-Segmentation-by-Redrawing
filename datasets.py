...
class Dataset:
    ...
    def transform(image_path: str, label_path: str) -> Tuple[tf.Tensor,
        ...
        label = tf.cast(
            tf.where(label >= object_threshold, 1, 0), tf.uint8)[:, :, 0]
    ...
    def get_split(self, split: str, size: int=None, batch_size: int=25,
        ...
        for i, path in enumerate(self.paths):
            if self.split_keys[i] == Dataset.SPLIT_KEYS[split]:
                split_image_paths.append(
                    os.path.join(self.root, self.image_dir, ('image_' + path)))
                split_label_paths.append(
                    os.path.join(self.root, self.label_dir, ('label_' + path)))
        ...
        if size is not None:
            subset_indices = tf.random.uniform(
                [size, 1], 0, split_image_paths.shape[0], dtype=tf.dtypes.int64)
            ...
            split_ds = split_ds.shuffle(buffer_size=tf.cast(
                tf.math.ceil(
                    tf.convert_to_tensor(
                        len(split_image_paths) / batch_size)),
                tf.int64))
    ...
    def get_split(self, split: str, size: int=None, batch_size: int=25,
        ...
        for i, path in enumerate(self.paths):
            if Dataset.SPLIT_KEYS[split] == self.split_keys[i]:
                split_image_paths.append(
                    os.path.join(self.root, self.image_dir, (path + '.jpg')))
                split_label_paths.append(
                    os.path.join(self.root, self.label_dir, (path + '.png')))
...
class FlowerDataset(Dataset):
    ...
    def transform(image_path: str, label_path: str) -> Tuple[tf.Tensor,
        ...
        label = tf.cast(
            tf.where(
                tf.logical_or(
                    label <= 0.9*background_color,
                    label >= 1.1*background_color),
                1, 0),
            tf.uint8)[:, :, 0]
        label = tf.one_hot(label, depth=2)
...
class FaceDataset(Dataset):
    ...
    def get_split(self, split: str, size: int=None, batch_size: int=25,
        ...
        for i, path in enumerate(self.paths):
            if Dataset.SPLIT_KEYS[split] == self.split_keys[i]:
                split_image_paths.append(os.path.join(
                    self.root, self.image_dir, (path[:-9] + '/' + path)))
                if split == 'training' or split == 'validation':
                    split_label_paths.append(os.path.join(
                        self.root, self.image_dir, (path[:-9] + '/' + path)))
                else:
                    split_label_paths.append(
                        os.path.join(self.root, self.label_dir, path))
    ...
    def transform(image_path: str, label_path: str) -> Tuple[tf.Tensor,
        ...
        object_threshold = 40
        label = tf.cast(tf.where(
            label > object_threshold, 1, 0), tf.uint8)[:, :, 0]
        label = tf.one_hot(label, depth=2)

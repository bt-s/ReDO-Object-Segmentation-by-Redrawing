...
def main(args):
    ...
    # generator object containing class generator objects for each class and
    # information conservation network
    generator = Generator(n_classes=args.n_classes, n_input=args.n_input,
                          init_gain=args.init_gain,
    ...
    # define optimizer
    g_optimizer = Adam(learning_rate=1e-4, beta_1=0,
                       beta_2=0.9)  # optimizer for the generator
    d_optimizer = Adam(learning_rate=1e-4, beta_1=0,
                       beta_2=0.9)  # optimizer for the discriminator
    i_optimizer = Adam(learning_rate=1e-4, beta_1=0,
                       beta_2=0.9)  # optimizer for the information network
    f_optimizer = Adam(learning_rate=1e-5, beta_1=0,
                       beta_2=0.9)  # optimizer for the segmentation network


    # define metrics dictionary
    metrics = {'g_d_loss_train': Mean(), 'g_i_loss_train': Mean(),
               # generator loss: discriminator | information
    ...
    def discriminator_update(batch_images_real, training):
        ...
        with tf.GradientTape() as tape:
            ...
            batch_images_fake = models['G'](
                batch_images_real, batch_masks_logits, update_generator=False,
                training=training
            )
        ...
        if training:
            ...
            d_optimizer.apply_gradients(
                zip(d_gradients, models['D'].trainable_variables)
            )
    ...
    def generator_update(batch_images_real, training):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            g_loss_d, g_loss_i = adversarial_loss.get_g_loss(
                d_logits_fake, batch_z_k, batch_z_k_hat
            )
        ...
        if training:
            ...
            i_gradients = tape.gradient(
                g_loss_i, models['I'].trainable_variables
            )

            # update weights
            g_optimizer.apply_gradients(
                zip(g_gradients, models['G'].trainable_variables)
            )
            f_optimizer.apply_gradients(
                zip(f_gradients, models['F'].trainable_variables)
            )
            i_optimizer.apply_gradients(
                zip(i_gradients, models['I'].trainable_variables)
            )
        ...
        for phase in phases:
            ...
            for batch_id, (batch_images_real, _) in enumerate(datasets[phase]):
                ...
                print('Batch: {:d}/{:d}'.format(
                    batch_id + 1,
                    tf.data.experimental.cardinality(datasets[phase])
                ))
